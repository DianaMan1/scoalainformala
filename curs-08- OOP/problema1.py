...
class Extensie1(Catalog):
    ...
    def final_grade(self):
        ...
        note_finale = {}
        for cheie, valoare in self.materii.items():
            if all(isinstance(x, int) for x in valoare):
                medie = sum(valoare) / len(valoare)
                note_finale.update({cheie: medie})
        return note_finale
