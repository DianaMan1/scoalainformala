...
def get_year_data(data_set, year):
   index = description[1].index(year)

   value = []

   for country in data_set:
       country_data = {country[0]: country[1][index]}
       value.append(country_data)

   return {year: value}


def get_country_data(data_set, country_index):
   search_result = None

   for country in data_set:
       country_name = country[0]
       if country_name == country_index:
...
def perform_average(data_set, country_index):
   country_data = get_country_data(data_set, country_index)
   country_values = list(map(to_numeric, country_data[1]))
   return sum(country_values) / len(country_values)


def main():
   print(get_year_data(dataset, "2019"))
   print(get_country_data(dataset, "RO"))
   print(perform_average(dataset, "RO"))
